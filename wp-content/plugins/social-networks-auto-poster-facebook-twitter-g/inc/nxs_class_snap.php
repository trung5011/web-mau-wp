       ...
       <?php /* ################## WHAT URL to USE */ ###################### ?>
          ...
          foreach ($nxs_snapAvNts as $avNt) { $clName = 'nxs_snapClass'.$avNt['code'];
             ...
             $cbo = 0; $cboInx = 0; $jXj = 0; foreach ($ntClInst->nt as $indx=>$pbo){ 
               ...
               $pbo['hideMe'] = !empty($options['hideUnchecked']) && (empty($pbo['do']) || $fltPostTypeExcl ); if (!$pbo['hideMe']) $jXj++;  
             } if ($cbo==0) continue; if (!$this->sMode['t']) $cbo = 1;
             if ($cbo==1) { ?>
               <?php  $pbo = $ntClInst->nt[$cboInx]; $pbo['jj']=$cboInx; $pbo['cbo']=$cbo; $pbo['hideMe'] = !empty($options['hideUnchecked']) && (empty($pbo['do']) || $fltPostTypeExcl ); 
